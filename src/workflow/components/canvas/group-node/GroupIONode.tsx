...
function DataTypeDot({ dataType }: { dataType?: string }) {
  ...
  return <span className={`w-1.5 h-1.5 rounded-full ${color} flex-shrink-0 opacity-60`} />;
}
...
function InlineParamPicker({
  ...
  const groupParams = (groupNode?.data?.params ?? {}) as Record<string, unknown>;
  ...
  const handleToggle = useCallback(
    (subNodeId: string, subNodeLabel: string, paramKey: string, dataType: string) => {
      if (isExposed(subNodeId, paramKey)) {
        const ep = exposedList.find((p) => p.subNodeId === subNodeId && p.paramKey === paramKey);
        if (ep) unexposeParam(groupId, ep.namespacedKey, direction);
      } else {
        exposeParam(groupId, {
          subNodeId, subNodeLabel, paramKey,
      ...
      {childNodes.length === 0 ? (
        ...
        childNodes.map((child) => {
          ...
          const modelSchema = (child.data?.modelInputSchema ?? []) as Array<{
            name: string; label?: string; type?: string; mediaType?: string;
          }>;
          const paramDefs = (child.data?.paramDefinitions ?? []) as Array<{
            key: string; label: string; dataType?: string;
          }>;
          ...
          const exposedCount = items.filter((it) => isExposed(child.id, it.key)).length;
          const isCollapsed = collapsedSections.has(child.id);

          return (
            <div key={child.id} className="rounded-lg overflow-hidden bg-black/[0.06] dark:bg-white/[0.04]">
              <button
              ...
              >
                ...
                <span className="text-[10px] font-semibold text-foreground/80 truncate flex-1 text-left">{fullLabel}</span>
                {exposedCount > 0 && (
                  <span className={`text-[9px] font-bold px-1.5 rounded-full ${isInput ? "bg-cyan-500/20 text-cyan-400" : "bg-emerald-500/20 text-emerald-400"}`}>
                    {exposedCount}
                  </span>
                )}
                <span className="text-[8px] text-muted-foreground/30 font-mono flex-shrink-0">{shortId}</span>
              </button>
              {!isCollapsed && (
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => handleToggle(child.id, fullLabel, item.key, item.dataType)}
                      ...
                      >
                        <div className={`w-3.5 h-3.5 rounded flex-shrink-0 flex items-center justify-center transition-all ${
                          exposed ? (isInput ? "bg-cyan-500" : "bg-emerald-500") : "border border-muted-foreground/20"
                        }`}>
                          {exposed && (
                            <Check className="w-2 h-2 text-white" strokeWidth={3} />
                          )}
                        </div>
                        <span className={`flex-1 text-left truncate ${exposed ? "font-medium" : ""}`}>{item.label}</span>
...
function GroupIONodeComponent({ data, id }: NodeProps<GroupIONodeData>) {
  ...
  const ports = useMemo(
    () =>
      exposedParams.map((ep) => {
        const label = ep.alias || ep.paramKey
          .split("_")
  ...
  const portFingerprint = useMemo(() => ports.map((p) => p.key).join(","), [ports]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (pickerOpen) document.body.setAttribute("data-subgraph-picker-open", "true");
    else document.body.removeAttribute("data-subgraph-picker-open");
    return () => { document.body.removeAttribute("data-subgraph-picker-open"); };
  }, [pickerOpen]);
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") { e.stopPropagation(); e.preventDefault(); setPickerOpen(false); }
    };
  ...
  const handleRemoveParam = useCallback((nk: string) => {
  ...
  const getHandleY = (index: number) =>
    ACCENT_STRIP_HEIGHT + BORDER_W + HEADER_HEIGHT + index * PORT_ROW_HEIGHT + PORT_ROW_HEIGHT / 2;

  return (
    <div ref={cardRef} className="relative" style={{ width: isInput ? NODE_WIDTH : NODE_WIDTH_OUTPUT }}>
      ...
      {ports.map((port, i) => (
        <Handle
          ...
          style={{
            ...
            position: "absolute",
            top: getHandleY(i),
            ...(isInput ? { right: -(HANDLE_SIZE / 2) } : { left: -(HANDLE_SIZE / 2) }),
      ...
      >
        ...
        >
          <div className={`rounded-md p-1 flex-shrink-0 ${isInput ? "bg-cyan-500/15" : "bg-emerald-500/15"}`}>
            {isInput
              ? <ArrowRightFromLine className="w-3.5 h-3.5 text-cyan-400" strokeWidth={2.5} />
              : <ArrowLeftToLine className="w-3.5 h-3.5 text-emerald-400" strokeWidth={2.5} />}
          </div>

          <span className={`text-[12px] font-bold tracking-wide flex-1 min-w-0 truncate ${isInput ? "text-cyan-500 dark:text-cyan-400" : "text-emerald-600 dark:text-emerald-400"}`}>
            {isInput ? t("workflow.groupInput", "Group Input") : t("workflow.groupOutput", "Group Output")}
          </span>

          {ports.length > 0 && (
            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${isInput ? "bg-cyan-500/15 text-cyan-500 dark:text-cyan-400" : "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400"}`}>
              {ports.length}
            </span>
          )}
            ...
            <TooltipTrigger asChild>
              <button
                className={`
                  ...
                  ${pickerOpen
                    ? (isInput ? "bg-cyan-500 border-cyan-500 text-white" : "bg-emerald-500 border-emerald-500 text-white")
                    : (isInput ? "bg-cyan-500 border-cyan-500 text-white hover:bg-cyan-600 hover:border-cyan-600" : "bg-emerald-500 border-emerald-500 text-white hover:bg-emerald-600 hover:border-emerald-600")
                  }
                `}
                onClick={(e) => { e.stopPropagation(); setPickerOpen(!pickerOpen); }}
              >
                <Settings2 className="w-3 h-3" />
                <span>{pickerOpen ? t("workflow.done", "Done") : t("workflow.configure", "Configure")}</span>
              </button>
            </TooltipTrigger>
        ...
        {(ports.length > 0 || pickerOpen) && (
          <div className={`mx-2 h-px ${isInput ? "bg-cyan-500/15" : "bg-emerald-500/15"}`} />
        )}
        ...
        {ports.length === 0 && !pickerOpen ? (
          <div className="flex items-center justify-center py-5 px-3">
            ...
            >
              ...
              {isInput ? t("workflow.addInputs", "Add Inputs") : t("workflow.addOutputs", "Add Outputs")}
            </button>
          </div>
        ) : ports.length > 0 ? (
          <div className="py-1">
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isHovered
                      ? (isInput ? "bg-cyan-500/[0.06]" : "bg-emerald-500/[0.06]")
                      : ""
                    }
                  `}
                ...
                >
                  ...
                  <div
                    className={`absolute top-1 bottom-1 w-[2px] rounded-full transition-opacity ${
                      isHovered
                        ? (isInput ? "bg-cyan-500/60 opacity-100" : "bg-emerald-500/60 opacity-100")
                        : "opacity-0"
                    } ${isInput ? "left-0" : "right-0"}`}
                  />

                  <div className={`flex items-center gap-1.5 flex-1 min-w-0 px-3 ${isInput ? "" : "flex-row-reverse"}`}>
                    ...
                    {isEditing ? (
                      <input
                        ...
                        onKeyDown={(e) => {
                          ...
                          if (e.key === "Escape") { e.stopPropagation(); setEditingAlias(null); }
                        }}
                      ...
                      <span className={`flex-1 min-w-0 text-[11px] font-medium truncate text-foreground/80 ${isInput ? "text-left" : "text-right"}`}>
                        {port.label}
                      </span>
                    )}
                        ...
                        <Tooltip delayDuration={0}>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); startAliasEdit(port.ep.namespacedKey, port.ep.alias ?? ""); }}
                            >
                        ...
                        <Tooltip delayDuration={0}>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); handleRemoveParam(port.ep.namespacedKey); }}
                            >
        ...
        {/* ── Inline picker ───────────────────────────────────── */}
        {pickerOpen && (
          <div className={`border-t ${isInput ? "border-cyan-500/15" : "border-emerald-500/15"}`}>
